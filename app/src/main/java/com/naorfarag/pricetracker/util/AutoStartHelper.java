...
public class AutoStartHelper {
    ...
    private void autoStartAsus(final Context context) {
        if (isPackageExists(context, PACKAGE_ASUS_MAIN)) {
            ...
            showAlert(context, sweetAlertDialog -> {
                try {
                    PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
    ...
    private void showAlert(Context context, SweetAlertDialog.OnSweetClickListener onClickListener) {
        ...
        sweetAlertDialog
                .setTitleText("Enabling auto-start is mandatory to track price fluctuations at all times!\nPlease enable it through settings")
    ...
    private void autoStartLetv(final Context context) {
        if (isPackageExists(context, PACKAGE_LETV_MAIN)) {
            showAlert(context, sweetAlertDialog -> {
                ...
                try {
                    PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                    AutoStartHelper.this.startIntent(context, PACKAGE_LETV_MAIN, PACKAGE_LETV_COMPONENT);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
    ...
    private void autoStartHonor(final Context context) {
        if (isPackageExists(context, PACKAGE_HONOR_MAIN)) {
            showAlert(context, sweetAlertDialog -> {
                ...
                try {
                    PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                    AutoStartHelper.this.startIntent(context, PACKAGE_HONOR_MAIN, PACKAGE_HONOR_COMPONENT);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
    ...
    private void autoStartOppo(final Context context) {
        if (isPackageExists(context, PACKAGE_OPPO_MAIN) || isPackageExists(context, PACKAGE_OPPO_FALLBACK)) {
            showAlert(context, sweetAlertDialog -> {
                ...
                try {
                    PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                    AutoStartHelper.this.startIntent(context, PACKAGE_OPPO_MAIN, PACKAGE_OPPO_COMPONENT);
                } catch (Exception e) {
                    e.printStackTrace();
                    try {
                        PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                        AutoStartHelper.this.startIntent(context, PACKAGE_OPPO_FALLBACK, PACKAGE_OPPO_COMPONENT_FALLBACK);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                        try {
                            PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                            AutoStartHelper.this.startIntent(context, PACKAGE_OPPO_MAIN, PACKAGE_OPPO_COMPONENT_FALLBACK_A);
                        } catch (Exception exx) {
                            exx.printStackTrace();
                        }
    ...
    private void autoStartVivo(final Context context) {
        if (isPackageExists(context, PACKAGE_VIVO_MAIN) || isPackageExists(context, PACKAGE_VIVO_FALLBACK)) {
            showAlert(context, sweetAlertDialog -> {
                ...
                try {
                    PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                    AutoStartHelper.this.startIntent(context, PACKAGE_VIVO_MAIN, PACKAGE_VIVO_COMPONENT);
                } catch (Exception e) {
                    e.printStackTrace();
                    try {
                        PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                        AutoStartHelper.this.startIntent(context, PACKAGE_VIVO_FALLBACK, PACKAGE_VIVO_COMPONENT_FALLBACK);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                        try {
                            PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                            AutoStartHelper.this.startIntent(context, PACKAGE_VIVO_MAIN, PACKAGE_VIVO_COMPONENT_FALLBACK_A);
                        } catch (Exception exx) {
                            exx.printStackTrace();
                        }
    ...
    private void autoStartNokia(final Context context) {
        if (isPackageExists(context, PACKAGE_NOKIA_MAIN)) {
            showAlert(context, sweetAlertDialog -> {
                ...
                try {
                    PrefUtil.setBooleanPreference(context, Finals.AUTO_START, true);
                    AutoStartHelper.this.startIntent(context, PACKAGE_NOKIA_MAIN, PACKAGE_NOKIA_COMPONENT);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
